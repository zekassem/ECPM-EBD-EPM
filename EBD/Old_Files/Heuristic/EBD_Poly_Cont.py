...
no_dist_list=[10,50] # the number of districts
...
class Model(): # This is the model where we add the variables and the constraints
    ...
    def branch_bound_cut(self): # This is the method that has the branch and bound and cut
        ...
        d=c_1 # you can comment this code
    ...
    def solve_poly_cont(self):
        ...
        c_logic.solve()
        l_1 = round(time.time() - t0, 2)
        sol_1 = [x_v[i] for i, j in enumerate(c_logic.solution.get_values(x_v)) if j > 0.01]
        sol_2 = [w_v[i] for i, j in enumerate(c_logic.solution.get_values(w_v)) if j > 0.01]
...
for prob in probs_list:
    ...
    for num in no_dist_list:
        for tol in tol_list:
            ...
            with open('Results/EBD_Cut_Set_vs_SP_Contiguity_Cut_1_no_dist_'+str(num)+'_prob_'+str(prob)+'.csv','w') as newFile:
                ...
                obj, l_1,l_2, d = model_bbb.branch_bound_cut()
