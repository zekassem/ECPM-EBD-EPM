...
# Importing .dat file
prob=3001
data="data_"+str(prob)+"_SanDiego.csv"
...
coordinates= np.loadtxt(data,skiprows=4,max_rows=no_nodes1,usecols=(1,2), delimiter=',') # skipping lines to get the x & y coordinates of the nodes
...
z=s+4 # Calculating the lines needed to be skipped to get adjacency list
adj_list=np.loadtxt(data,skiprows=z,max_rows=no_edges1,usecols=(0,1),dtype=int, delimiter=',')
...
no_dist=[4] # the number of districts
t=0.1 # tolerance parameter into the balancing constraints
