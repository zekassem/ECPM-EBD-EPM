...
def constraints_wo_cuts(problem, Graph, no_dist, x_v, w_v): #  This function adds all of the constraints for the original districting problem
    ...
    expr = [cplex.SparsePair(x_v[(e - 1):Graph.no_edges * Graph.no_nodes:Graph.no_edges],[1 for i in range(Graph.no_nodes)]) for e in Graph.edges_list]
    sens = ["E"] * len(expr)
    rhs = [1] * len(expr)
...
class Model(): # This is the model where we add the variables and the constraints
    ...
    def branch_bound_cut(self):  # This is the method that has the branch and bound and cut
        ...
        for o in center_node:
            result.append([Graph.edge_e[i] for i, val in enumerate(d.solution.get_values(x_v)) if
        ...
        while a > 0:
            ...
            if sum(C) < len(center_node):
                ...
                for o in center_node:
                    result.append(
                        [Graph.edge_e[i] for i, val in enumerate(d.solution.get_values(x_v)) if
                         val > 0.01 and Graph.node_i[i] == o])
            else:
                a = 0
            print('num_cuts')
    ...
    def solve_poly_cont(self):
        ...
        print(obj)
        print('time solve with SP Contiguity')
        print(l_1)
        return obj,l_1,c
...
def execute_task(task):
    ...
    with open('Results/Cut_Empty/'+str(no_threads)+'_threads/PM_Cut_Set_vs_SP_Contiguity_no_dist_'+str(num)+'_prob_'+str(prob)+'.csv','w') as newFile:
