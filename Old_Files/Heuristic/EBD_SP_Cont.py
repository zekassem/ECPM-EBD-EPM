...
class Graph():
    def __init__(self, prob):
        ...
        print(self.euc_dist_edge_index)
...
def logic_cuts(Graph,problem,w_v,x_v): # function that adds the logic cuts
    ...
    expr = [cplex.SparsePair([x_v[(q - 1) + (i - 1) * Graph.no_edges] for q in Graph.edges[i]] + [w_v[i - 1]],
                             [1 for r in range(len(Graph.edges[i]))] + [-2]) for i in Graph.nodes_list]
    sens = ["G"] * len(expr)
    rhs = [0] * len(expr)
    problem.linear_constraints.add(lin_expr=expr, senses=sens, rhs=rhs)
    return problem
...
def SP_contiguity_const(Graph,x_v,problem): # Function that adds the contiguity constraints: x_i,(j,k)<= x_i,(l,m) where (l,m) \in SP_i,(j,k) and (l,m) \in Cutset({(j,k)}) \forall i \in V, \forall (j,k) \in E
    expr = [cplex.SparsePair([x_v[index], x_v[(node_i[index] - 1) * Graph.no_edges + (
...
class Model(): # This is the model where we add the variables and the constraints
    ...
    def solve_poly_cont(self):
        ...
        c_logic.parameters.clocktype.set(2)
        c_logic.parameters.timelimit.set(43200)
        c_logic.solve()
        l_1 = round(time.time() - t0, 2)
        ...
        obj = c_logic.solution.get_objective_value()
        print('Solution')
        print(sol_1)
        print(sol_2)
        print('Objective Function')
        print(obj)
        return obj,l_1,c_logic

probs_list=['CARP_F11_g_graph.dat']
    ...
    for num in no_dist_list:
        for tol in tol_list:
            ...
            with open('Results/PM_Cut_Set_vs_SP_Contiguity_Cut_1_no_dist_'+str(num)+'_prob_'+str(prob)+'.csv','w') as newFile:
                ...
                newFileWriter.writerow(['Computation Time_before_BBB', 'Objective Function','num_iterations','num_cuts'])
                ...
                newFileWriter.writerow([l_1 , round(obj, 2),num_iterations,num_cuts])
                newFileWriter.writerow(['Computation Time_for_BBB'])
                ...
                newFileWriter.writerow(['Computation Time_W_Cut_1_SP_Cont', 'Objective Function'])
