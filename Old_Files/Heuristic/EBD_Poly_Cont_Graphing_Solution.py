...
class Graph():
    def __init__(self,prob):
        ...
        self.coordinates = np.loadtxt(self.file, skiprows=4, max_rows=self.no_nodes1, usecols=(1, 2),
        ...
        self.coordinates_tuple = list(
            zip(self.x_coordinate, self.y_coordinate))  # creating a tuple for the x and y-coordinate
...
def Graphing_Solution(Graph,no_dist,tol,formulation_type,center_node,distircts_edges):
    ...
    nx.draw(G, pos=pos, with_labels=True, node_size=10, edge_color=edge_color_list, node_color=node_color_list)
...
def heuristic(Graph,center_node,districts_edges,tol,num):
    ...
    print('This is the start of Heuristic')
    for o in center_node:
        ...
        print('center_node')
        print(o)
        edges_corr = [graph.nodes_corr[l] for l in districts_edges[o]]
        print('edges_corr')
        print(edges_corr)
        print('cut set')
        corr_cut_set=[graph.nodes_corr[l] for l in connect_edges_neighboring_n]
        print(corr_cut_set)
        ...
        for e in connect_edges_neighboring_n:
            ...
            if d_ue_value[0]<d_oe_value[0] and new_district_o_total_demand>=LB and new_district_u_total_demand<=UB:
                print('nodes_corr')
                print(Graph.nodes_corr[e])
                print('old_center')
                print(o)
                print('new_center')
                print(u)
    ...
    for num in no_dist_list:
        for tol in tol_list:
            ...
            with open('Results/EBD_Cut_Set_vs_SP_Contiguity_Cut_1_no_dist_'+str(num)+'_tol_'+str(tol)+'_prob_'+str(prob)+'.csv','w') as newFile:
                ...
                obj, l_1, d,center_node_1,districts_edges_1,districts_nodes_1 = model_bbb.branch_bound_cut()
                ...
                for o in center_node_1:
                    center_node_index=o
                    nodetoedge_net_dist, nodetoedge_path=Dijkstras_Algo(graph,center_node_index,districts_edges_1[center_node_index],districts_nodes_1[center_node_index])
                    nodetoedge_net_dist_all_1.update(nodetoedge_net_dist)
                    nodetoedge_net_path_all_1.update(nodetoedge_path)
                Graphing_Solution(graph, num, tol, formulation_type_1, center_node_1, districts_edges_1)

                print('heuristic function')
                heuristic(graph,center_node_1,districts_edges_1,tol,num)
                print('district_edges')
                print(districts_edges_1)

                #(Graph, center_node, districts_edges)
                ...
                for o in center_node_2:
                    center_node_index=o
                    nodetoedge_net_dist, nodetoedge_path=Dijkstras_Algo(graph,center_node_index,districts_edges_2[center_node_index],districts_nodes_2[center_node_index])
                    nodetoedge_net_dist_all_2.update(nodetoedge_net_dist)
                    nodetoedge_net_path_all_2.update(nodetoedge_path)
                Graphing_Solution(graph, num, tol, formulation_type_2,center_node_2, districts_edges_2)
                ...
                print('LB')
                print(LB)
                print('Distance from center 16 to edge'+str(graph.nodes_corr[36]))
                print(graph.nodetoedge_net_dist.iloc[[15],[37]])
                print('Distance from center 60 to edge'+ str(graph.nodes_corr[36]))
                print(graph.nodetoedge_net_dist.iloc[[59], [37]])
