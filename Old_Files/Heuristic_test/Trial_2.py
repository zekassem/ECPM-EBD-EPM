...
def execute_task(task):
    ...
    graph_edges=[]
    for i in range(len(graph.from_node_i)):
        graph_edges.append((graph.from_node_i[i]+1,graph.to_node_j[i]+1,graph.edge_lengths[i]))
    ...
    with open('Results/Cut_Empty/' + str(no_threads) + '_threads/EBD_Cut_Set_vs_SP_Contiguity_no_dist_' + str(
            num) + '_tol_' + str(tol) + '_prob_' + str(prob) + '.csv',
              'w') as newFile:
        ...
        try:
            ...
            # new_obj = heuristic(graph, center_node, districts_edges, tol, num, obj)
            # print('new_obj')
            # print(new_obj)
