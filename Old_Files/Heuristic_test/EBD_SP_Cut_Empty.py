...
class Model(): # This is the model where we add the variables and the constraints
    ...
    def branch_bound_cut(self): # This is the method that has the branch and bound and cut
        ...
        return obj,l_1,l_2,num_iterations, num_cuts,d,center_node,districts_edges
    ...
    def solve_poly_cont(self):
        ...
        return obj,l_1,c,center_node,districts_edges
...
def execute_task(task):
    ...
    with open('Results/Cut_Empty/'+str(no_threads)+'_threads/EBD_Cut_Set_vs_SP_Contiguity_no_dist_' + str(num) +'_tol_'+str(tol)+ '_prob_' + str(prob) + '.csv',
        ...
        try:
            ...
            obj, l_1, l_2, num_iterations, num_cuts, d,center_node_cut,districts_edges_cut = model_bbb.branch_bound_cut()
            print('Cut Set Obj')
            print(obj)
            ...
            formulation_type_cut='Cut_Set'
            # Graphing_Solution(graph, num, tol, formulation_type_cut, center_node_cut, districts_edges_cut)
        ...
        try:
            ...
            obj, l_1, c,center_node_SPC,districts_edges_SPC = model_sp_cont.solve_poly_cont()
            print('old_SP_Obj')
            print(obj)
            print('Start of Heuristic')
            new_obj=heuristic(graph, center_node_SPC, districts_edges_SPC, tol, num, obj)
            print('new_obj')
            print(new_obj)
            ...
            formulation_type_SPC = 'SP_Contiguity'
            # Graphing_Solution(graph, num, tol, formulation_type_SPC, center_node_SPC, districts_edges_SPC)

            for o in districts_edges_cut:
                for j in districts_edges_cut[o]:
                    if j not in districts_edges_SPC[o]:
                ...
                for k in districts_edges_SPC[o]:
                    if k not in  districts_edges_cut[o]:
                        print('SP District')
...
task=['EBD_SP_Cut_Empty','CARP_N17_g_graph.dat',6,0.1]
execute_task(task)